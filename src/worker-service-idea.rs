...
// Function to chunk a Vec<KeyValue>
fn chunk_key_values(data: &Vec<KeyValue>, chunk_size: usize) -> Vec<Vec<KeyValue>> {
...
impl MapReduce for MapReduceWorker {
    ...
    async fn gen_chunks(
        ...
        print!("reduce_chunk");
        let output = self.context.chunking_function_rayon_serialized(function,data);
        match output {
            Some(x) => {
        ...
        match result_job {
            Ok((stream, addr)) => {
                ...
                tokio::spawn(async move {
                    ...
                    //let chunk_result =  process_chunk_job(/*&context,*/ job, &worker_address, sender, worker_addresses).await;
                    
                    // Send the job to the worker for processing
                    let chunk_result = match process_chunk_job(/*&context,*/ job, &worker_address, sender.clone(), worker_addresses.clone()).await {
                        Ok(result) => result,
                        Err(e) => {
                            ...
                            return; // Stop the async block
                        }
                    };

                    let map_results = match process_map_job(chunk_result, sender.clone(), worker_addresses.clone()).await {
                        Ok(result) => result,
                        Err(e) => {
                            eprintln!("Error processing job: {:?}", e);
                            let _ = sender.send(Job {
                    ...
                    let reduce_result = process_reduce_job(map_results, &worker_address, sender, worker_addresses);
                    if let Err(e) = reduce_result.await {
                        eprintln!("Error processing job: {:?}", e);
                    }
                });
            }
...
async fn process_chunk_job(
    ...
    sender: mpsc::Sender<Job>,
    worker_addresses: Arc<Mutex<HashMap<SocketAddr, (MapReduceClient<Channel>, bool)>>>,
) -> Result<Vec<Vec<KeyValue>>, Box<dyn std::error::Error>> {
    // Create a new client for each job to avoid data races
    let mut context = Context::new();
    add_function_chunk!(context, chunk_key_values);
    ...
    let chunks = chunking_distributed!(context, chunk_key_values, job_data, &mut client);
...
async fn process_map_job(
    chunks: Vec<Vec<KeyValue>>,
    sender: mpsc::Sender<Job>,
    worker_addresses: Arc<Mutex<HashMap<SocketAddr, (MapReduceClient<Channel>, bool)>>>,
) -> Result<Vec<KeyValue>, Box<dyn std::error::Error>> {
    let mut handles = vec![];

    // Create a new channel to collect map results
    let (map_sender, map_receiver) = mpsc::channel::<Vec<KeyValue>>(chunks.len()); // Adjust the buffer size as needed
    ...
    for chunk in chunks {
        let sender_clone = map_sender.clone();
        ...
        let handle = tokio::spawn(async move {
            let mut client: MapReduceClient<Channel> = if let Some((client, _)) = worker_addresses_clone
                .lock()
                .unwrap()
                .get_mut(&worker_address)
            {
                ...
                return Err(Box::new("Worker not found".into()) as Box<dyn std::error::Error + Send + 'static>);
            };

            // Perform the map function for the given chunk
            let map_result = map_distributed!(context_clone, multiply_by_2, chunk, &mut client);
            println!("Map RESPONSE={:?}", map_result);

            // Send the map result to the channel
            sender_clone.send(map_result).await.unwrap();
    ...
    // Close the channel to signal that all map results have been sent
    drop(map_sender);

    // Collect map results from the channel
    let mut map_results = vec![];
    while let Some(result) = map_receiver.recv().await {
        map_results.push(result);
    }

    Ok(map_results.into_iter().flatten().collect())
}

async fn process_reduce_job(
    map_results: Vec<KeyValue>,
    worker_address: &SocketAddr,
    sender: mpsc::Sender<Job>,
    worker_addresses: Arc<Mutex<HashMap<SocketAddr, (MapReduceClient<Channel>, bool)>>>,
) -> Result<KeyValue, Box<dyn std::error::Error>> {
    // Create a new client for each job to avoid data races
    let mut context = Context::new();
    ...
    // Perform the reduce function for all mapped results
    let reduce_result = reduce_distributed!(context, sum, map_results, &mut client);
